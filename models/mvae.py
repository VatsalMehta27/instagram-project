...
class MVAE(nn.Module):
    def __init__(
        self, device, hidden_dim, latent_size, text_embeddings, img_embedding_size
    ):
        ...
        self.word_embeddings = nn.Embedding(
            self.vocab_size, self.text_embedding_size
        ).from_pretrained(embeddings=torch.FloatTensor(text_embeddings), freeze=True)
    ...
    def forward(self, text_input, img_input):
        ...
        dec_text_fc_result = dec_text_fc_result.repeat(self.text_embedding_size).view(
    ...
    def loss_function(
        ...
        loss = (
            img_recon_loss + text_recon_loss + kl_divergence + binary_classifier_loss
        ) / decoded_text.shape[0]
